    ...
    def from_new_request(
        ...
        if lmcache_cached_tokens > 0:
            ...
            print(
                ...
                f"unfolded_block_ids_len={len(unfolded_block_ids)}",
                flush=True,
            )
...
class ReqMeta:
    ...
    def from_request_tracker(
        ...
        if len(token_ids) > num_blocks * block_size:
            ...
            print(
                "### LMCache_REQMETA_LEN_DEBUG ###",
                f"req_id={tracker.req_id}",
                f"len(token_ids)={len(token_ids)}",
                f"num_blocks={num_blocks}",
                f"block_size={block_size}",
                f"max_slots={num_blocks * block_size}",
                f"input_token_len={input_token_len}",
                f"num_saved_tokens={tracker.num_saved_tokens}",
                f"is_last_prefill={is_last_prefill}",
                f"skip_save={skip_save}",
                f"load_spec={'none' if load_spec is None else f'can_load={load_spec.can_load},lmcache_cached_tokens={load_spec.lmcache_cached_tokens},vllm_cached_tokens={load_spec.vllm_cached_tokens}'}",
                flush=True,
            )
...
class LMCacheConnectorV1Impl:
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        for idx, request in enumerate(metadata.requests):
            ...
            if len(tokens) != len(slot_mapping):
                ...
                selected_tokens = getattr(request, "selected_tokens", None)
                slot_shape = tuple(slot_mapping.shape) if hasattr(slot_mapping, "shape") else None
                slot_numel = slot_mapping.numel() if hasattr(slot_mapping, "numel") else None
                print(
                    "### LMCache_MISMATCH ###",
                    f"file={__file__}",
                    f"req_id={getattr(request, 'req_id', None)}",
                    f"len(tokens)={len(tokens)}",
                    f"len(slot_mapping)={len(slot_mapping)}",
                    f"slot_mapping_shape={slot_shape}",
                    f"slot_mapping_numel={slot_numel}",
                    "load_spec=",
                    (None if load_spec is None else
                    f"(can_load={load_spec.can_load} lmcache_cached_tokens={load_spec.lmcache_cached_tokens} vllm_cached_tokens={load_spec.vllm_cached_tokens})"),
                    f"selected_tokens_is_none={selected_tokens is None}",
                    flush=True,
                )
                raise AssertionError(
                    f"### MISMATCH_ASSERT_REACHED ### len(tokens)({len(tokens)}) != len(slot_mapping)({len(slot_mapping)})"
                )
            ...
            if self.use_layerwise:
                ...
                if self.enable_blending:
                    ...
                    layerwise_retriever = self.lmcache_engine.retrieve_layer_head_token_wise(
                        tokens[:lmcache_cached_tokens], # not really needed
        ...
        for request in metadata.requests:
            ...
            if self.enable_sparse_attention:
                ...
                if not selected_tokens:
                    selected_tokens = [i for i in range(len(request.token_ids))]
                ret_token_mask = layerwise_retriever.send(selected_tokens)
            else:
    ...
    def build_connector_meta(
        ...
        for request in scheduler_output.scheduled_new_reqs:
            ...
            if load_spec is not None and load_spec.can_load:
                print(
                    "### LMCache_BUILD_META_DEBUG ###",
                    f"req_id={request.req_id}",
                    f"num_computed_tokens={request.num_computed_tokens}",
                    f"num_scheduled_tokens={scheduler_output.num_scheduled_tokens[request.req_id]}",
                    f"num_tokens_to_compute={num_tokens_to_compute}",
                    f"load_spec_can_load={load_spec.can_load}",
                    f"load_spec_lmcache_cached_tokens={load_spec.lmcache_cached_tokens}",
                    f"load_spec_vllm_cached_tokens={load_spec.vllm_cached_tokens}",
                    f"tracker_token_ids_len={len(request_tracker.token_ids)}",
                    f"tracker_allocated_block_ids_len={len(request_tracker.allocated_block_ids)}",
                    flush=True,
                )
